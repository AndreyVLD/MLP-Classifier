...
def he_normal_init(shape: tuple[int, int]) -> np.ndarray:
    return np.random.randn(*shape) * np.sqrt(2 / shape[1])


def lecun_normal_init(shape: tuple[int, int]) -> np.ndarray:
    return np.random.randn(*shape) * np.sqrt(1 / shape[1])


def xavier_normal_init(shape: tuple[int, int]) -> np.ndarray:
    return np.random.randn(*shape) * np.sqrt(2 / (shape[1] + shape[0]))


def xavier_uniform_init(shape: tuple[int, int]) -> np.ndarray:
    val_range = math.sqrt(6) / math.sqrt(shape[0] + shape[1])
    return np.random.uniform(-val_range, val_range, shape)


def he_uniform_init(shape: tuple[int, int]) -> np.ndarray:
    limit = math.sqrt(6 / shape[1])
    return np.random.uniform(-limit, limit, shape)


def orthogonal_init(shape: tuple[int, int]) -> np.ndarray:
